...
public interface MatchingMapper {
	...
	//マッチング検索
	@Select("SELECT * ,((CASE WHEN matching_gender = #{matching_gender} THEN 5 ELSE 0 END) +(CASE WHEN matching_time = #{matching_time} THEN 4 ELSE 0 END) +(CASE WHEN matching_member = #{matching_member} THEN 3 ELSE 0) + (CASE WHEN ABS((#{minAge} + #{maxAge}) /2 - (matching_min_age + matching_max_age) /3 ) <= 5 THEN 3 ELSE 0 END) + (CASE WHEN matching_area = #{matching_area} AND matching_area != 'どこでもよい' AND #{matching_area} = 'どこでもよい' THEN 1 ELSE 0 END)) AS matching_score FROM matching WHERE user_id != #{user_id} AND user_id NOT IN (SELECT blocking_user_id FROM blocking WHERE user_id = #{user_id}) AND matching_day = #{matching_day} AND matching_max_age >= #{minAge} AND matching_min_age <= #{maxAge} AND matching_member = #{matching_member} AND (matching_area = #{matching_area} OR matching_area = 'どこでもよい' OR #{matching_area} = 'どこでもよい') ORDER BY matching_score DESC  LIMIT 3")
